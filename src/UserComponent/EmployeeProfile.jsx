...
const EmployeeProfile = () => {
  ...
  const downloadResume = async () => {
    try {
      // pehle fresh data lo database se
      const userResponse = await axios.get(
        "http://localhost:8080/api/user/fetch?userId=" + fetchedEmployee.id
      );

      const freshResume = userResponse.data.users[0].userProfile.resume;
      ...
      // ab fresh resume naam se download karo
      const response = await axios.get(
        `http://localhost:8080/api/user/employee/resume/${freshResume}/download?employeeId=${fetchedEmployee.id}`,
        { responseType: "blob" }
      );
